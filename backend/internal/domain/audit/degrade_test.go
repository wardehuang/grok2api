...
func TestClassifyOutputSpeed(t *testing.T) {
	tests := []struct {
		...
		{name: "late first token with reasoning uses tail and sums tokens", output: 1511, reasoning: 1400, first: 19763, duration: 19827, soft: 500, hard: 1000, minGen: 1000, wantClass: DegradeClassHard},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.name == "late first token with reasoning uses tail and sums tokens" {
				if genMS != test.duration-test.first {
					t.Fatalf("genMS = %d, want tail %d", genMS, test.duration-test.first)
				}
				want := float64(test.output+test.reasoning) * 1000 / float64(test.duration-test.first)
				if tps != want {
...
func TestGenerationWindowMSUsesDurationMinusFirstToken(t *testing.T) {
	if got := GenerationWindowMS(19763, 19827); got != 64 {
