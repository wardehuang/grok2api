...
const accountValidator = hasShape({
  ...
  egressNodeId: isOptional(isString), egressAssignmentMode: isOptional(isOneOf("manual", "auto")), egressExitIp: isOptional(isString),
  lastRefreshErrorStatus: isOptional(isNumber), lastRefreshErrorCode: isOptional(isString), lastRefreshErrorMessage: isOptional(isString), lastRefreshErrorResponse: isOptional(isString), priority: isNumber, schedulingOrder: isOptional(isNumber), maxConcurrent: isNumber, minimumRemaining: isNumber,
